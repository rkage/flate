...
- **Diff output isn't an applyable patch.** `flate diff` defaults to dyff path-keyed syntax (`@@ <path> @@`); GitHub's diff lexer renders it natively. Pass `-o unified` for the standard `---/+++/@@` line format — useful for reading and for tools that consume unified-diff lines, but not framed as a multi-file patch (constant `from`/`to` labels, no `diff --git` framing), so don't feed it to `git apply` / `patch`. List ordering in unified mode is positional; the K8s-aware list-by-identifier matching is dyff-only.
